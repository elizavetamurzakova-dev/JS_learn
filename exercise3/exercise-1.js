...
class Player{
    ...
    takeTurn(opponent) {
        let xInput = prompt("Координата Х:");
        if (xInput === null) return null;

        let x = +xInput;
        if (isNaN(x) || !Number.isInteger(x)) {
            alert("Координата X должна быть целым числом");
            return null;
        }

        let yInput = prompt("Координата Y:");
        if (yInput === null) return null;

            let y = +yInput;
            if (isNaN(y) || !Number.isInteger(y)) {
                alert("Координата Y должна быть целым числом");
            return null;
        }
        if (x < 0 || x >= opponent.boardSize) {
            alert(`Координата X должна быть от 0 до ${opponent.boardSize - 1}`);
            return null;
        }
        if (y < 0 || y >= opponent.boardSize) {
            alert(`Координата Y должна быть от 0 до ${opponent.boardSize - 1}`);
            return null;
        }
...
let input = prompt('Введите имя игрока и размер поля через пробел:');

let arr = input.split(' ');
let player = new Player(arr[0], parseInt(arr[1]));
